...
//                           [--source tiles|layers] [--seams snap|overlap] [--nodes 0] [--rm] [--dpr 1] [--quality 90]
...
// draws the full renders. The camera is hard-clamped like the client.
...
function parse(argv) {
  const o = { out: null, cx: null, cy: null, z: null, vw: 1920, vh: 1080, tier: 'HIGH', t: 0, source: fs.existsSync(path.join(__dirname, 'roblox/manifest.json')) ? 'tiles' : 'layers', nodes: '1', rm: false, dpr: 1, shots: null, seams: 'snap', quality: 90 };
  const pos = [];
...
async function main() {
  ...
  try {
    ...
    for (const s of shots) {
      ...
      console.log(`${s.out} (${(fs.statSync(s.out).size / 1024).toFixed(0)} KB): C (${st.C.x.toFixed(0)}, ${st.C.y.toFixed(0)}) z ${st.z.toFixed(3)}  rift ${st.w.rift.toFixed(2)} corrupt ${st.w.corrupt.toFixed(2)}  ` +
